...
// ***************************************************************************** 
 ...
 void LCDClearScreen_xy(int x_0, int x_1, int y_0, int y_1)
 {
  long i; // loop counter 
  
  // Row address set (command 0x2B) 
  WriteSpiCommand(PASET); //PASET NA PAGEADDRSET
  WriteSpiData(x_0); //poczatek obszaru na osi x
  WriteSpiData(x_1); //koniec obszaru na osi x
  
  // Column address set (command 0x2A) 
  WriteSpiCommand(CASET); //CASET na COLADDRSET
  WriteSpiData(y_0); //poczatek obszaru na osi y
  WriteSpiData(y_1); //koniec obszaru na osi y
...
// use this information to output three data bytes
WriteSpiData((Word0 >> 4) & 0xFF);
WriteSpiData(((Word0 & 0xF) << 4) | ((Word1 >> 8) &
0xF));
WriteSpiData(Word1 & 0xFF);
}
}
...
/*void LCDBlindText(int x, int y, int size, int w, int cColor) {
// loop counter
int i;
// Row address set (command 0x2B)
WriteSpiCommand(PASET);
...
void drawBoard()
{
LCDSetLine(1, 2, 125, 2, BLACK); //Pionowa 1
LCDSetLine(1, 3, 125, 3, BLACK);
LCDSetLine(1, 4, 125, 4, BLACK);

LCDSetLine(1, 129, 125, 129, BLACK); //Pionowa 2
LCDSetLine(1, 128, 125, 128, BLACK);
LCDSetLine(1, 127, 125, 127, BLACK);

LCDSetLine(2, 2, 2, 125, BLACK); //Pozioma 1
LCDSetLine(3, 2, 3, 125, BLACK);
LCDSetLine(4, 2, 4, 125, BLACK);

LCDSetLine(129, 2, 129, 125, BLACK); //Pozioma 2
LCDSetLine(128, 2, 128, 125, BLACK);
LCDSetLine(127, 2, 127, 125, BLACK);
}
